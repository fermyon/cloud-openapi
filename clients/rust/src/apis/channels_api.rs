...
/// struct for typed errors of method [`api_channels_id_healthz_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiChannelsIdHealthzGetError {
    UnknownValue(serde_json::Value),
}
...
pub async fn api_channels_id_healthz_get(configuration: &configuration::Configuration, id: &str, api_version: Option<&str>) -> Result<crate::models::HealthCheckResult, Error<ApiChannelsIdHealthzGetError>> {
    ...
    let local_var_uri_str = format!("{}/api/channels/{id}/healthz", local_var_configuration.base_path, id=crate::apis::urlencode(id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ApiChannelsIdHealthzGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
