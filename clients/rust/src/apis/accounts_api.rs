...
/// struct for typed errors of method [`api_accounts_id_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiAccountsIdGetError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`api_accounts_id_patch`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiAccountsIdPatchError {
    UnknownValue(serde_json::Value),
}
...
pub async fn api_accounts_id_get(configuration: &configuration::Configuration, id: &str, api_version: Option<&str>) -> Result<crate::models::AccountDetails, Error<ApiAccountsIdGetError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ApiAccountsIdGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn api_accounts_id_patch(configuration: &configuration::Configuration, id: &str, patch_account_details_command: crate::models::PatchAccountDetailsCommand, api_version: Option<&str>) -> Result<(), Error<ApiAccountsIdPatchError>> {
    ...
    local_var_req_builder = local_var_req_builder.json(&patch_account_details_command);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ApiAccountsIdPatchError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
